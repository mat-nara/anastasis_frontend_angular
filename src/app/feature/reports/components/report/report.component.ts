...
export class ReportComponent implements OnInit {
  ...
  constructor(
    ...
    loadReportData(version: string){
      ...
      this.reportService.getReports(version).subscribe(
        (data: any) => {
          ...
          this.setDisplayedColumns('rotation');
    ...
    updatePropDirrapro(){
      console.log(this.changedDirapproQECBoiteList);
    ...
    handleTabsReport(event: any, selected : string){

      var toogle_btn = document.querySelectorAll('#report-toogle-tabs .toogle-btn')! as NodeListOf<HTMLElement>;

      toogle_btn.forEach((btn) => { btn.classList.remove('active') });
      event.currentTarget .classList.add('active');
      
      this.setDisplayedColumns(selected);
    }

    setDisplayedColumns(selected: string){

      if(!['rotation', 'decision', 'collisage'].includes(selected)){
        selected = 'rotation';
      }
